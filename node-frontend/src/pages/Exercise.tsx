...
export default function Exercise() {
  ...
  useEffect(() => {
    ...
    if (isTesting) {
      ...
      cameraRef.current.start().catch((err: Error) => {
        console.error("Camera error in test mode:", err);
        alert("Failed to start camera in test mode. Check permissions: " + err.message);
      });
      return () => {
        if (cameraRef.current) cameraRef.current.stop();
      };
    ...
    pose.onResults((results: Results) => {
      ...
      // Lighting check
      const avgBrightness = getAverageBrightness(video);
      setLighting(avgBrightness >= BRIGHTNESS_THRESHOLD ? "Good" : "Poor");

      // Rep counting logic can go here (dummy example)
      const leftWristY = results.poseLandmarks[15].y;
      const leftShoulderY = results.poseLandmarks[11].y;
    ...
    cameraRef.current.start().catch((err: Error) => {
      console.error("Camera error:", err);
      alert("Failed to start camera. Check permissions: " + err.message);
    });

    return () => {
      if (cameraRef.current) cameraRef.current.stop();
    };
  }, [exerciseName, navigate, isTesting]);

  return (
    <div style={{ textAlign: "center" }}>
      <h1>Exercise: {exerciseName}</h1>
      <video ref={videoRef} style={{ display: "none" }} playsInline></video>
      <canvas ref={canvasRef} width="640" height="480" style={{ border: "2px solid #333" }}></canvas>
      <div>
        <p>Stage: {stage}</p>
        <p>Reps: {repCount}</p>
